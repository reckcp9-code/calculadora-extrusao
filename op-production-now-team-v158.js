(function(){
  'use strict';
  if(window.DFProductionNowTeamV159)return;
  window.DFProductionNowTeamV159=true;
  ...
  function currentTeam(){
    try{
      if(window.DFOpCloud&&typeof window.DFOpCloud.team==='function'){
        const live=window.DFOpCloud.team();
        if(live)return live;
      }
    }catch(e){}
    const saved=loadJson(TEAM_KEY,null);
    if(saved)return saved;
    const box=$('dfOpTeamCloud'),txt=box&&String(box.textContent||'');
    if(/\bDONO\b/i.test(txt))return{role:'owner'};
    if(/\bOPERADOR\b/i.test(txt))return{role:'operator'};
    return null;
  }
  const teamNow=()=>currentTeam();
  const canEdit=()=>String(currentTeam()?.role||'').toLowerCase()==='owner';
  ...
  function migrate(){
    try{
      ...
      if(test!==null)localStorage.setItem(STATUS_KEY,test);
        ...
        if(Array.isArray(a))localStorage.setItem(STATUS_KEY,JSON.stringify(a.map(x=>({machineKey:String(x.id||x.machine||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),machine:x.machine||'',status:x.status||'RODANDO',product:x.product||'',op:x.op||'',operator:x.operator||'',startedAt:x.startedAt||now(),updatedAt:x.updatedAt||now()}))));
      }
    }catch(e){}
  }
  ...
  function opOptions(){const r=registry(),out=[];Object.keys(r).forEach(k=>{const raw=r[k]||{},id=String(raw.id||k||'');if(!id.startsWith('DFOP-'))return;out.push({...raw,id})});return out.sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')))}
  ...
  function showOpInfo(){const s=$('dfNowOpSelect'),e=$('dfNowOpInfo');if(!s||!e)return;const x=registry()[s.value],p=x&&x.expected||{};if(!x){e.textContent='Selecione uma OP para puxar automaticamente o código DFOP e o produto.';return}e.textContent='Código: '+s.value+' • Produto: '+(p.title||'—')+(p.largura?' • Largura: '+p.largura:'')+(p.micra?' • Micra: '+p.micra:'')}

  function build(){const edit=canEdit(),role=edit?'<div class="dfNowOwnerOk">👑 DONO RECONHECIDO — você pode definir o que está rodando.</div>':'';const admin=edit?`<div id="dfNowAdminForm"><h4>DEFINIR O QUE ESTÁ RODANDO</h4><div class="grid"><div><label>MÁQUINA</label><input id="dfNowMachine" placeholder="Ex.: Máquina 1"></div><div><label>STATUS</label><select id="dfNowStatus"><option>RODANDO</option><option>PARADA</option><option>AJUSTE</option><option>TROCA DE MATERIAL</option><option>FINALIZADA</option></select></div><div style="grid-column:1/-1"><label>OP GERADA</label><select id="dfNowOpSelect"></select><div id="dfNowOpInfo" class="dfNowOpInfo"></div></div><div><label>OPERADOR</label><input id="dfNowOperator" placeholder="Nome do operador"></div><div><label>INÍCIO</label><input id="dfNowStart" type="time"></div></div><button id="dfNowSave" type="button">✅ PUBLICAR STATUS</button></div>`:`<div class="dfNowReadOnly">👁️ Somente visualização. Somente o dono da equipe pode definir o que está rodando.</div>`;return `<div class="dfNowHead"><h3>🏭 PRODUÇÃO AGORA</h3><p>Veja o que está rodando nas máquinas neste momento.</p>${role}<div class="dfNowKpis"><div class="dfNowKpi"><span>RODANDO</span><b id="dfNowRunning">0</b></div><div class="dfNowKpi"><span>PARADAS</span><b id="dfNowStopped">0</b></div><div class="dfNowKpi"><span>AJUSTE</span><b id="dfNowAdjust">0</b></div></div><div id="dfNowCloudMsg"></div></div><div id="dfNowList"></div>${admin}`}

  function bindAdmin(){if(!canEdit())return;const b=$('dfNowSave'),sel=$('dfNowOpSelect'),st=$('dfNowStart');if(b)b.onclick=saveForm;if(sel)sel.onchange=showOpInfo;if(st&&!st.value)st.value=new Date().toTimeString().slice(0,5);fillOps()}
  function mount(){style();const ops=$('dfFormulaOps'),tabs=ops&&ops.querySelector('.dfOpsTabs');if(!ops||!tabs)return false;let tab=$('dfNowTab');if(!tab||!tabs.contains(tab)){if(tab&&tab.parentNode)tab.parentNode.removeChild(tab);tab=document.createElement('button');tab.id='dfNowTab';tab.dataset.pane='now';tab.textContent='🏭 AGORA';tabs.appendChild(tab)}else if(tab!==tabs.lastElementChild){tabs.appendChild(tab)}let pane=$('dfPaneNow');const mode=canEdit()?'owner':'view';if(!pane||!ops.contains(pane)){if(pane&&pane.parentNode)pane.parentNode.removeChild(pane);pane=document.createElement('div');pane.id='dfPaneNow';pane.className='dfOpsPane';ops.appendChild(pane)}if(pane.dataset.mode!==mode){pane.dataset.mode=mode;pane.innerHTML=build()}tab.onclick=openNow;bindAdmin();render();syncCloud();return true}
  function openNow(){mount();document.querySelectorAll('#dfFormulaOps .dfOpsTabs button').forEach(b=>b.classList.toggle('on',b.id==='dfNowTab'));document.querySelectorAll('#dfFormulaOps .dfOpsPane').forEach(p=>p.classList.remove('on'));$('dfPaneNow')?.classList.add('on');render();syncCloud()}
  ...
  function render(){const list=$('dfNowList');if(!list)return;const a=activeList(),owner=canEdit();if($('dfNowRunning'))$('dfNowRunning').textContent=a.filter(x=>x.status==='RODANDO').length;if($('dfNowStopped'))$('dfNowStopped').textContent=a.filter(x=>x.status==='PARADA').length;if($('dfNowAdjust'))$('dfNowAdjust').textContent=a.filter(x=>x.status==='AJUSTE'||x.status==='TROCA DE MATERIAL').length;list.innerHTML=a.length?a.sort((x,y)=>String(x.machine).localeCompare(String(y.machine))).map(x=>{const c=sc(x.status),acts=owner?`<div class="dfNowActions"><button data-edit="${esc(x.machineKey)}">✏️ EDITAR</button><button class="done" data-done="${esc(x.machineKey)}">⏹ FINALIZAR</button></div>`:'';return `<div class="dfNowCard ${c}"><div class="dfNowTop"><div class="dfNowMachine">${esc(x.machine||'Máquina')}</div><span class="dfNowBadge ${c}">${esc(x.status)}</span></div><div class="dfNowProduct">${esc(x.product||'Sem produto')}</div><div class="dfNowMeta">OP: <b>${esc(x.op||'—')}</b><br>Operador: <b>${esc(x.operator||'—')}</b><br>Início: <b>${hm(x.startedAt)}</b> • Atualizado: <b>${hm(x.updatedAt)}</b></div>${acts}</div>`}).join(''):'<div class="dfOpsStatus">Nenhuma máquina marcada como ativa agora.</div>';if(owner){list.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>edit(b.dataset.edit));list.querySelectorAll('[data-done]').forEach(b=>b.onclick=()=>finish(b.dataset.done))}}
  function machineKey(v){return String(v||'').trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
  async function commit(rec){if(!canEdit())return;const a=load().filter(x=>x.machineKey!==rec.machineKey);a.unshift(rec);save(a);setCloudMsg('⏳ Publicando para a equipe...','');try{if(await publishCloud(rec)){setCloudMsg('✅ Status publicado para a equipe','ok');setTimeout(syncCloud,500)}else setCloudMsg('⚠️ Sem equipe conectada. Salvo apenas neste aparelho.','warn')}catch(e){setCloudMsg('⚠️ Não consegui publicar agora.','warn')}}
  function saveForm(){if(!canEdit())return;const machine=String($('dfNowMachine').value||'').trim(),status=$('dfNowStatus').value,op=$('dfNowOpSelect').value,operator=String($('dfNowOperator').value||'').trim();if(!machine)return alert('Digite a máquina.');if(status==='RODANDO'&&!op)return alert('Selecione a OP que está rodando.');const ent=registry()[op]||{},exp=ent.expected||{},t=$('dfNowStart').value||new Date().toTimeString().slice(0,5),d=new Date(),p=t.split(':');d.setHours(+p[0]||0,+p[1]||0,0,0);const key=machineKey(machine)||('m-'+Date.now()),old=load().find(x=>x.machineKey===key);const rec={machineKey:key,machine,status,op:op||'',product:exp.title||old?.product||'',operator,startedAt:old&&old.status==='RODANDO'&&status==='RODANDO'?old.startedAt:d.toISOString(),updatedAt:now()};commit(rec);$('dfNowMachine').value='';$('dfNowOperator').value='';$('dfNowStatus').value='RODANDO';$('dfNowStart').value=new Date().toTimeString().slice(0,5);fillOps()}
  function edit(key){if(!canEdit())return;const x=load().find(v=>v.machineKey===key);if(!x)return;$('dfNowMachine').value=x.machine||'';$('dfNowStatus').value=x.status||'RODANDO';$('dfNowOperator').value=x.operator||'';fillOps(x.op||'');try{$('dfNowStart').value=new Date(x.startedAt||Date.now()).toTimeString().slice(0,5)}catch(e){}$('dfNowAdminForm')?.scrollIntoView({behavior:'smooth',block:'center'})}
  function finish(key){if(!canEdit())return;const x=load().find(v=>v.machineKey===key);if(!x)return;commit({...x,status:'FINALIZADA',updatedAt:now()})}

  function scheduleMount(){setTimeout(mount,100)}
  function boot(){migrate();purgeSynthetic();const mo=new MutationObserver(()=>{purgeSynthetic();hideSyntheticCloudRows();if(!$('dfNowTab')||!$('dfPaneNow'))scheduleMount()});mo.observe(document.documentElement,{childList:true,subtree:true});let n=0,t=setInterval(()=>{if(mount()||++n>60)clearInterval(t)},250);window.addEventListener('df-ui-ready',scheduleMount);window.addEventListener('df-team-changed',scheduleMount);window.addEventListener('df-team-joined',scheduleMount);window.addEventListener('pageshow',scheduleMount);window.addEventListener('focus',scheduleMount);window.addEventListener('df-op-remote-merged',()=>{purgeSynthetic();setTimeout(hideSyntheticCloudRows,50)});window.addEventListener('online',syncCloud);document.addEventListener('click',e=>{if(e.target?.id==='dfFormTabOps')scheduleMount()},true);setInterval(()=>{purgeSynthetic();hideSyntheticCloudRows();if(!document.hidden){mount();syncCloud()}},5000)}
