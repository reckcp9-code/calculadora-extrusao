(function(){
  'use strict';
  if(window.DFOpTeamCodeSafeTestV164)return;
  window.DFOpTeamCodeSafeTestV164=true;
  ...
  function patch(){
    ...
    let line=$('dfTeamCodeSafeTest');
    if(!line){
      line=document.createElement('div');line.id='dfTeamCodeSafeTest';
    ...
    if(hasCloudSession()){
      ...
      const r=$('dfCloudRefresh');if(r){r.disabled=false;r.style.opacity='';}
    }else{
      note.textContent='⚠️ Este link do navegador está sem a sessão automática da nuvem. O código continua disponível e o restante da tela não fica preso tentando recuperar acesso.';
