...
function run(force){recover(!!force).then(function(){notifyReady()}).catch(function(){})}
function boot(){var n=0,iv=setInterval(function(){n++;if(secret()){clearInterval(iv);run(false)}else if(n>80)clearInterval(iv)},150);window.addEventListener('online',function(){run(true)});window.addEventListener('pageshow',function(){setTimeout(function(){run(false)},300)})}
