...
function notifyReady(){try{window.dispatchEvent(new CustomEvent('df-gerador-owner-ready'))}catch(e){}[80,500,1400].forEach(function(ms){setTimeout(function(){try{if(window.DFGeradorTeamPermissions&&typeof window.DFGeradorTeamPermissions.load==='function')window.DFGeradorTeamPermissions.load()}catch(e){}try{if(window.DFGeradorTeamDevices&&typeof window.DFGeradorTeamDevices.refresh==='function')window.DFGeradorTeamDevices.refresh(true)}catch(e){}},ms)})}
function run(force){if(!secret())return;recover(!!force).then(function(){notifyReady()}).catch(function(){})}
function boot(){var n=0,iv=setInterval(function(){n++;if(secret()){clearInterval(iv);run(false)}else if(n>120)clearInterval(iv)},150);window.addEventListener('df-admin-secret-ready',function(){setTimeout(function(){run(true)},100)});window.addEventListener('df-access-recovered',function(){setTimeout(function(){run(false)},250)});window.addEventListener('df-ios-access-restored',function(){setTimeout(function(){run(false)},250)});window.addEventListener('online',function(){run(true)});window.addEventListener('pageshow',function(){setTimeout(function(){run(false)},300)})}
