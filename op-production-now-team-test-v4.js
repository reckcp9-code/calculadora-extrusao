(function(){
  'use strict';
  if(window.DFProductionNowTeamTestV4)return;
  window.DFProductionNowTeamTestV4=true;

  var TEAM_KEY='df_op_team_v1';
  var REG_KEY='df_op_qr_registry_v1';
  var STATUS_KEY='df_production_now_team_test_v4';
  var OLD_KEYS=['df_production_now_team_test_v3','df_production_now_team_test_v2','df_production_now_v1'];
  var ensureTimer=0;
  ...
  function owner(){var t=currentTeam();return !!(t&&String(t.role||'').toLowerCase()==='owner')}

  function migrate(){
    try{
      if(localStorage.getItem(STATUS_KEY)!==null)return;
      for(var i=0;i<OLD_KEYS.length;i++){
        var raw=localStorage.getItem(OLD_KEYS[i]);
        if(raw!==null){var a=JSON.parse(raw||'[]');if(Array.isArray(a)){saveJson(STATUS_KEY,a);return}}
      }
    }catch(e){}
  }

  function addStyle(){
    if($('dfProdNowTestV4Style'))return;
    var s=document.createElement('style');s.id='dfProdNowTestV4Style';s.textContent=
      ...
      '.dfNowOwnerOk{margin-top:9px;border:1px solid #166534;background:#052e16;color:#86efac;border-radius:11px;padding:9px 10px;font-size:11px;font-weight:900}'+
  ...
  function opOptions(){
    var r=registry(),out=[];
    Object.keys(r).forEach(function(k){var raw=r[k]||{},id=String(raw.id||k||'');if(id.indexOf('DFOP-')!==0)return;var x={};Object.keys(raw).forEach(function(p){x[p]=raw[p]});x.id=id;out.push(x)});
  ...
  function showOpInfo(){var s=$('dfNowOpSelect'),e=$('dfNowOpInfo');if(!s||!e)return;var x=registry()[s.value],p=x&&x.expected||{};if(!x){e.textContent='Selecione uma OP para puxar automaticamente o código DFOP e o produto.';return}e.textContent='Código: '+s.value+' • Produto: '+(p.title||'—')+(p.largura?' • Largura: '+p.largura:'')+(p.micra?' • Micra: '+p.micra:'')}

  function build(){
    var edit=owner();
    var role=edit?'<div class="dfNowOwnerOk">👑 DONO RECONHECIDO — você pode definir o que está rodando.</div>':'';
    var admin=edit?'<div id="dfNowAdminForm"><h4>DEFINIR O QUE ESTÁ RODANDO</h4><div class="grid"><div><label>MÁQUINA</label><input id="dfNowMachine" placeholder="Ex.: Máquina 1"></div><div><label>STATUS</label><select id="dfNowStatus"><option>RODANDO</option><option>PARADA</option><option>AJUSTE</option><option>TROCA DE MATERIAL</option><option>FINALIZADA</option></select></div><div style="grid-column:1/-1"><label>OP GERADA</label><select id="dfNowOpSelect"></select><div id="dfNowOpInfo" class="dfNowOpInfo"></div></div><div><label>OPERADOR</label><input id="dfNowOperator" placeholder="Nome do operador"></div><div><label>INÍCIO</label><input id="dfNowStart" type="time"></div></div><button id="dfNowSave" type="button">✅ DEFINIR PRODUÇÃO AGORA</button><div class="dfNowTestNote">TESTE: esta tela ainda não publica para os outros celulares.</div></div>':'<div class="dfNowReadOnly">👁️ Somente visualização. Somente o dono da equipe pode definir o que está rodando.</div>';
  ...
  function ensure(){
    ...
    if(pane.getAttribute('data-mode')!==mode){pane.setAttribute('data-mode',mode);pane.innerHTML=build()}
    bind();render();return true;
  }
  function queueEnsure(){clearTimeout(ensureTimer);ensureTimer=setTimeout(ensure,30)}

  function boot(){
    migrate();addStyle();queueEnsure();
    var mo=new MutationObserver(queueEnsure);mo.observe(document.documentElement,{childList:true,subtree:true});
    window.addEventListener('df-ui-ready',queueEnsure);window.addEventListener('df-team-changed',queueEnsure);window.addEventListener('df-team-joined',queueEnsure);window.addEventListener('pageshow',queueEnsure);window.addEventListener('focus',queueEnsure);
    document.addEventListener('click',function(e){var t=e.target;if(t&&(t.id==='dfFormTabOps'||(t.closest&&t.closest('#dfFormTabOps'))))setTimeout(queueEnsure,60)},true);
