(function(){
  'use strict';
  if(window.DFManualQrCodeTestV2)return;
  window.DFManualQrCodeTestV2=true;
  ...
  function addStyle(){
    if($('dfManualQrCodeTestV2Style'))return;
    const s=document.createElement('style');
    s.id='dfManualQrCodeTestV2Style';
    s.textContent=`
  ...
  function hookSave(form){
    const btn=form&&form.querySelector('#dfManualSave');if(!btn||btn.dataset.dfManualQrHook==='2')return;
    btn.dataset.dfManualQrHook='2';
    btn.addEventListener('click',function(e){
      const field=$('dfManualQrCode');if(!field)return;
      let v=validateCode(true);
