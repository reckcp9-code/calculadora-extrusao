(function(){
  ...
  function resinRowsText(f,srv){
    const items=Array.isArray(srv?.itens)?srv.itens:[];
    return activeRows(f).map((r,i)=>{
      const m=mat(r.id)||r,it=items[i]||{};
      return String(m.nome||r.nome||'Material')+' — '+fm(r.pct||0,2)+'% — '+fm(it.kg||0,3)+' kg';
    }).join('\n');
  }
  ...
  async function shareWhatsApp(){
    ...
    const msg='*DF EXTRUSOR PRO – FORMULAÇÃO DE RESINAS*\n\n'+
      '*FORMULAÇÃO: '+String(f.nome||'Formulação')+'*\n'+
      '*BASE: 100 KG*\n\n'+
      resinRowsText(f,srv)+'\n\n'+
  ...
  function mount(){
    ...
    wrap.innerHTML='<div class="savedBtns" style="grid-template-columns:repeat(2,1fr);margin-top:8px"><button id="foWhatsResina" class="miniBtn" type="button">WHATSAPP</button><button id="foPdfResina" class="miniBtn" type="button">RESINAS PDF</button></div><div class="formNote" style="margin-top:6px">Relatório e WhatsApp usam sempre a base padrão de 100 kg, com Fator KG somente das resinas.</div>';
