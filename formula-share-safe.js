  ...
  function rowsHtml(f,srv){
    ...
    activeRows(f).forEach((r,i)=>{
      ...
      out+='<tr>'+ 
        ...
        '<td>'+fm(r.pct||0,2)+'%</td>'+ 
        '<td>'+fm(it.kg||0,3)+' kg</td>'+ 
        '<td>'+money(it.precoKg||0)+'</td>'+ 
        '<td>'+money(it.custo||0)+'</td>'+ 
      '</tr>';
    });
    return out||'<tr><td colspan="5">Sem materiais.</td></tr>';
  }
  ...
  async function pdfResinas(f,targetWin){
    ...
    const html='<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Relatório de Formulação - '+esc(f.nome||'')+'</title><style>'+ 
      ...
      '<main class="sheet">'+
        ...
        '<div class="rule"></div>'+ 
        '<div class="formula">'+esc(f.nome||'Formulação')+'</div>'+ 
        '<section class="cards">'+
          '<div class="card"><span>Quantidade total</span><b>100,000 kg</b></div>'+ 
          '<div class="card"><span>Total da porcentagem</span><b>'+fm(srv.totalPct||0,2)+'%</b></div>'+ 
          '<div class="card"><span>Custo total das resinas</span><b>'+money(srv.custoTotal||0)+'</b></div>'+ 
          '<div class="card"><span>Fator KG</span><b>'+money(srv.custoKgFinal||0)+'</b></div>'+ 
        '</section>'+ 
        '<div class="tableWrap"><table><thead><tr><th>Material</th><th>%</th><th>Kg</th><th>Valor/kg</th><th>Custo</th></tr></thead><tbody>'+rowsHtml(f,srv)+'</tbody></table></div>'+ 
        '<div class="footnote">Gerado em '+esc(gerado)+' • DF EXTRUSOR PRO</div>'+ 
        '<div class="note">Custo referente somente às resinas da formulação. Não inclui custo fabril, mão de obra, energia, extrusão, perdas, impostos ou demais despesas de fabricação.</div>'+ 
