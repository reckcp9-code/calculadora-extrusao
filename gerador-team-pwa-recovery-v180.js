...
function savePair(cred,dev,token){try{if(cred)localStorage.setItem(GEN_CRED_KEY,String(cred));if(dev)localStorage.setItem(GEN_DEVICE_KEY,String(dev));if(token)sessionStorage.setItem(TOKEN_KEY,String(token))}catch(e){}}
...
async function tryAdminTeam(){var s=secret();if(!s)return null;try{var r=await rawFetch(API+'/op/team/status',{method:'POST',headers:{'Content-Type':'application/json','X-DF-Admin':s},body:JSON.stringify({admin:true}),cache:'no-store'}),j={};try{j=await r.json()}catch(e){}if(r.ok&&j&&j.team)return{team:j.team,cred:'',dev:'',token:''}}catch(e){}return null}
async function recover(){
  ...
  try{
    ...
    if(local&&local.teamId){done=true;return}
    var s=secret();if(!s){setMsg('Aguardando o acesso administrativo para localizar sua equipe...','');return}
    setMsg('Buscando sua equipe pelo acesso administrativo...','');

    var direct=await tryAdminTeam();
    if(direct&&direct.team){saveTeam(direct.team);done=true;setMsg('Equipe: '+String(direct.team.name||'DF EXTRUSOR')+' • '+(String(direct.team.role||'').toLowerCase()==='owner'?'DONO':'OPERADOR'),'ok');try{window.DFGeradorTeamPermissions&&window.DFGeradorTeamPermissions.load&&window.DFGeradorTeamPermissions.load()}catch(e){}return}
    ...
    if(cached&&cached.teamId){done=true;setMsg('Equipe: '+String(cached.name||'DF EXTRUSOR')+' • '+(String(cached.role||'').toLowerCase()==='owner'?'DONO':'OPERADOR'),'ok');return}
