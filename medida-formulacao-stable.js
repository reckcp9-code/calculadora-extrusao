(function(){
  'use strict';
  if(window.DFMedidaFormulaStable)return;
  ...
  function ensureOverlay(){
    addStyle();if($('dfMedidaOverlay'))return;
    ...
    document.body.appendChild(d);$('dfMedidaMin').onclick=closeMedida;
  }
  ...
  function placeButton(){
    ...
    if(!row){
      ...
      if(label){holder.insertBefore(row,label);row.appendChild(label);label.style.margin='0'}else holder.insertBefore(row,nome);
    }
  ...
  function openMedida(ev){
    ...
    ensureOverlay();
    const pg=$('pgEx'),host=$('dfMedidaHost'),overlay=$('dfMedidaOverlay');if(!pg||!host||!overlay)return;
    if(opened)return;
    if(!marker){marker=document.createComment('df-pgEx-origin');pg.parentNode.insertBefore(marker,pg)}
    pgExWasOn=pg.classList.contains('on');bodyOverflow=document.body.style.overflow||'';
    host.appendChild(pg);pg.classList.add('on');overlay.hidden=false;overlay.scrollTop=0;document.body.style.overflow='hidden';opened=true;
  }

  function closeMedida(){
    if(!opened)return;
    const pg=$('pgEx'),overlay=$('dfMedidaOverlay');
    if(pg&&marker&&marker.parentNode){marker.parentNode.insertBefore(pg,marker.nextSibling);pg.classList.toggle('on',pgExWasOn)}
    if(overlay)overlay.hidden=true;
    document.body.style.overflow=bodyOverflow;opened=false;
