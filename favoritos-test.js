(function(){
  ...
  const TEST_KEY='df_favoritos_test_v1';
  const FAV_KEY='df_favoritos_v1';
  const TEST_PARAM='favoritosTeste';

  function $(id){return document.getElementById(id)}
  function enabled(){try{return localStorage.getItem(TEST_KEY)==='1'}catch(e){return false}}
  function enableFromUrl(){
    try{
      const u=new URL(location.href);
      if(u.searchParams.get(TEST_PARAM)==='1'){
        localStorage.setItem(TEST_KEY,'1');
        u.searchParams.delete(TEST_PARAM);
  ...
  function addStyle(){
    if($('dfFavTestStyle'))return;
    const st=document.createElement('style');
    st.id='dfFavTestStyle';
    st.textContent=[
      ...
      '.dfFavTestNote{margin-top:12px;border:1px solid #7c3aed;background:#1d1433;color:#e9d5ff;border-radius:12px;padding:10px;font-size:11px;line-height:1.4}',
  ...
  function ensurePage(){
    ...
    section.innerHTML='<div class="card"><span class="tag">Favoritos</span><h2>⭐ Meus favoritos</h2><div class="hint">Adicione as calculadoras que você mais usa para abrir direto sem ficar procurando.</div><div id="dfFavList" class="dfFavList"></div><div class="dfFavTestNote">🧪 TESTE PRIVADO — esta função está ativa somente neste aparelho para você testar antes de liberar para todos.</div><button id="dfFavDisable" class="calcBtn danger" type="button" style="margin-top:12px">DESATIVAR TESTE NESTE APARELHO</button></div>';
    const foot=app.querySelector('.foot');
    if(foot)foot.insertAdjacentElement('beforebegin',section);else app.appendChild(section);
    const off=$('dfFavDisable');if(off)off.onclick=function(){try{localStorage.removeItem(TEST_KEY)}catch(e){};location.reload()};
  }
  ...
  function ensure(){if(!enabled())return;addStyle();ensurePage();ensureButton();ensureStars();render()}
  function init(){enableFromUrl();if(!enabled())return;ensure();setTimeout(ensure,350);setTimeout(ensure,1100);document.addEventListener('visibilitychange',function(){if(!document.hidden)setTimeout(ensure,30)});if(location.hash==='#favoritos')setTimeout(showFavorites,500)}
