(function(){
  ...
  const savedCredential=String(localStorage.getItem(ACCESS_KEY)||'').trim();
  const hasAccess=!!linkToken||!!savedCredential;
  ...
  // EMERGÊNCIA MÓVEL: se este aparelho já tem credencial salva, não espera a rede/D1.
  // Libera a interface local imediatamente e tenta renovar em segundo plano.
  function unlockSavedImmediately(){
    if(linkToken||!savedCredential)return;
    setTimeout(function(){
    ...
    setTimeout(function(){if(localStorage.getItem(ACCESS_KEY))unlock()},180);
    setTimeout(function(){if(localStorage.getItem(ACCESS_KEY))unlock()},650);
  }
  ...
  async function renewAccess(){
    ...
    renewPromise=(async()=>{
      try{
        ...
        if(!r.ok||j.ok===false){
          // Só apaga acesso salvo quando o servidor confirma problema real de licença.
  ...
  async function redeemLegacy(){
    ...
    try{
      const r=await previousFetch(API+'/access/redeem',{
        method:'POST',headers:{'Content-Type':'application/json','X-DF-Device':deviceId()},
        body:JSON.stringify({token:linkToken,deviceId:deviceId()}),cache:'no-store'
      });
      let j={};try{j=await r.json()}catch(e){}
      ...
      setTimeout(unlock,120);setTimeout(unlock,650);return true;
  ...
  async function redeemIdentity(){
    ...
    try{
      const r=await previousFetch(API+'/access/identify',{
        method:'POST',headers:{'Content-Type':'application/json','X-DF-Device':deviceId()},
        body:JSON.stringify({identity,deviceId:deviceId()}),cache:'no-store'
      });
      let j={};try{j=await r.json()}catch(e){}
      if(!r.ok||j.ok===false)throw new Error(j.error||'Não foi possível liberar este acesso.');
  ...
  function prepareGeneralGate(){
    ...
    const keyInput=document.getElementById('licenseKey');const btn=document.getElementById('licenseBtn');if(!btn)return;
    ...
    if(!wrap){
      wrap=document.createElement('div');wrap.id='dfAccessIdentityWrap';wrap.style.cssText='width:100%;margin:12px 0 8px;text-align:left';
      const label=document.createElement('label');label.textContent='Sua identificação';label.setAttribute('for','dfAccessIdentity');label.style.cssText='display:block;margin:0 0 6px;font-size:12px;font-weight:800;color:#cbd5e1';
      const input=document.createElement('input');input.id='dfAccessIdentity';input.type='text';input.inputMode='text';input.autocomplete='username';input.placeholder='Ex.: seu WhatsApp ou código';input.style.cssText='width:100%;box-sizing:border-box;border:1px solid #475569;background:#111827;color:#fff;border-radius:12px;padding:12px;font:600 14px system-ui;outline:none';
    ...
    btn.textContent='ACESSAR';btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();redeemIdentity()},true);setMessage('Use sempre a mesma identificação.',true);
  }

  function prepareLegacyGate(){
    if(!linkToken||generalMode)return;
    const input=document.getElementById('licenseKey');const btn=document.getElementById('licenseBtn');if(!btn)return;
    if(input){input.value='';input.style.display='none'}setGateText('Seu acesso está pronto. Toque em ACESSAR para liberar este aparelho.');
    btn.textContent='ACESSAR';btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();redeemLegacy()},true);setMessage('Link de acesso válido para um único aparelho.',true);
  }

  async function resumeSavedAccess(){
    if(linkToken)return;
    const credential=String(localStorage.getItem(ACCESS_KEY)||'').trim();if(!credential)return;
    // A tela já foi liberada acima; esta parte apenas tenta sincronizar em segundo plano.
    const t=await renewAccess();
    if(t){setMessage('Acesso automático restaurado.',true);unlock();}
    else if(localStorage.getItem(ACCESS_KEY)){setMessage('Acesso salvo restaurado. Sincronização temporariamente indisponível.',true);unlock();}
  }
  ...
  function boot(){
    ...
    setTimeout(resumeSavedAccess,150);
  }

  // Executa cedo também no Safari/PWA, antes mesmo do DOMContentLoaded.
  unlockSavedImmediately();
