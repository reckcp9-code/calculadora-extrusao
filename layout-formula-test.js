(function(){
  ...
  function renameClient(){
    const nome=document.getElementById('foNome');
    if(!nome)return;
    ...
    const old=document.getElementById('dfOpenLit');if(old)old.remove();
    const modal=document.getElementById('dfLitModal');if(modal)modal.remove();
    const sel=document.getElementById('dfFoLitragem');if(sel&&sel.parentElement)sel.parentElement.remove();
    const mic=document.getElementById('dfFoMicra');if(mic&&mic.parentElement)mic.parentElement.remove();
  }
  ...
  function setup(){
    addStyle();removeLiteralNewline();renameClient();
    const page=document.getElementById('pgFo');
    ...
    if(!opsTop){
      opsTop=buttons.find(function(b){return norm(b.textContent).includes('BACKUP NA NUVEM')&&!b.dataset.dfBackupTop});
    ...
    if(!opsTop.dataset.dfBound){
      opsTop.dataset.dfBound='1';opsTop.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();nav.querySelectorAll('.dfAutoTopic').forEach(function(b){b.classList.toggle('on',b===opsTop)});page.querySelectorAll(':scope > .card').forEach(function(card){card.classList.remove('dfTopicVisible')});page.classList.remove('dfTestVendorOnly','dfTestBackupOnly');ops.click();window.scrollTo(0,0)},true);
    }
    if(!backupTop.dataset.dfBound){
      backupTop.dataset.dfBound='1';backupTop.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();nav.querySelectorAll('.dfAutoTopic').forEach(function(b){b.classList.toggle('on',b===backupTop)});page.querySelectorAll(':scope > .card').forEach(function(card){card.classList.toggle('dfTopicVisible',card===backupCard)});page.classList.remove('dfTestVendorOnly');page.classList.add('dfTestBackupOnly');window.scrollTo(0,0)},true);
    }
    if(!form.dataset.dfTestBound){
      form.dataset.dfTestBound='1';form.addEventListener('click',function(){nav.querySelectorAll('.dfAutoTopic').forEach(function(b){b.classList.remove('on')});page.classList.remove('dfTestVendorOnly','dfTestBackupOnly')});
    }
    syncScreens();return true;
  }
