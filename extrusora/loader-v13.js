...
try{
  const r=await fetch('./base-v9.html?v=15',{cache:'no-store'});
  if(!r.ok)throw new Error('Base indisponível');
  let html=await r.text();
  if(!html||!html.includes('</body>'))throw new Error('Base inválida');
  const extras=[
    '<script src="./projeto-rosca-v10.js?v=10.4"></script>',
    '<script src="./projeto-rosca-dimensionamento-v14.js?v=14.2"></script>',
    '<script src="./assistente-projeto-rosca-v15.js?v=15.1"></script>',
    '<script src="./projeto-rosca-ajuda-v12.js?v=12.4"></script>',
    '<script src="./relacao-polias-v11.js?v=11.4"></script>',
    '<script src="./diagnostico-tecnico-v9.js?v=9.5"></script>'
  ].join('');
