(function(){
  ...
  function writeCookie(name,value){if(!value)return;try{document.cookie=name+'='+encodeURIComponent(value)+'; Max-Age='+COOKIE_MAX_AGE+'; Path=/; SameSite=Lax; Secure'}catch(e){}}
  ...
  function newSecret(){
    try{const bytes=crypto.getRandomValues(new Uint8Array(32));let bin='';for(const b of bytes)bin+=String.fromCharCode(b);return btoa(bin).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'')}catch(e){return (crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).slice(2))+(crypto.randomUUID?crypto.randomUUID():Math.random().toString(36).slice(2))}
  }
