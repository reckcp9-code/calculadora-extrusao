(function(){
  ...
  let busy=false,lastHash='',timer=0,renewPromise=null;
  ...
  function setState(text,type){
    ...
    e.textContent=text;e.style.color=type==='ok'?'#86efac':type==='warn'?'#fde68a':'#94a3b8';
  }

  async function sync(){
    if(busy||document.hidden||navigator.onLine===false)return false;
    if(!localStorage.getItem(ACCESS_KEY))return false;
    const h=cloudHash();if(h===lastHash)return true;
    busy=true;
    try{
      ...
      snap.v=Math.max(2,Number(snap.v)||2);snap.createdAt=new Date().toISOString();snap.origin=location.origin;snap.opSyncVersion=2;
      await apiPost('/backup/save',{backup:await encrypt(snap,owner),clientTime:snap.createdAt});
      lastHash=h;setState('☁️ Dados das OPs sincronizados','ok');return true;
    }catch(e){setState('☁️ Dados das OPs aguardando sincronização','warn');return false}finally{busy=false}
  }
  ...
  async function restoreIfEmpty(){
    try{
      ...
      if(ops){localStorage.setItem(OPS_KEY,String(ops));if(reg)localStorage.setItem(REG_KEY,String(reg));lastHash=cloudHash();setState('☁️ Dados das OPs restaurados','ok');try{window.dispatchEvent(new CustomEvent('df-ui-ready'))}catch(e){}}
    }catch(e){}
  }

  function schedule(ms){clearTimeout(timer);timer=setTimeout(sync,ms||1000)}
  function boot(){
    setTimeout(()=>{restoreIfEmpty();schedule(1600)},900);
    window.addEventListener('df-op-qr-created',()=>schedule(900));
    window.addEventListener('online',()=>schedule(700));
    window.addEventListener('df-ui-ready',()=>schedule(1200));
    document.addEventListener('click',e=>{if(e.target&&e.target.closest&&e.target.closest('#dfManualSave'))schedule(1300)},true);
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule(500)});
    setInterval(()=>{if(!document.hidden&&cloudHash()!==lastHash)schedule(400)},20000);
  }

  window.DFOpSyncLite={sync:()=>sync()};
