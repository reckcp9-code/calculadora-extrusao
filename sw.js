...
const CORE=[
  ...
  './device-identity.js','./error-monitor-v1.js','./performance-guard-v1.js','./op-qr-hook.js','./app-bundle.js','./local-calculations.js',
...
const ESSENTIAL=new Set(['./index.html','./app-shell.html','./app-bundle.js','./local-calculations.js']);

function stateUrl(name){return new URL('__df_state_'+name+'__',self.registration.scope).href}
function cacheKey(input){const url=new URL(typeof input==='string'?input:input.url,self.registration.scope);url.search='';url.hash='';return new Request(url.href,{method:'GET'})}
...
async function fetchAndStore(cache,url){const req=new Request(new URL(url,self.registration.scope).href,{cache:'reload'}),res=await fetch(req,{cache:'no-cache'});if(!res||(res.ok!==true&&res.type!=='opaque'))throw new Error('Falha no cache: '+url);await cache.put(cacheKey(req),res.clone());return res}
async function precache(){
  const cache=await caches.open(DF_CACHE);
  const essential=CORE.filter(url=>ESSENTIAL.has(url));
  await Promise.all(essential.map(url=>fetchAndStore(cache,url)));
  await Promise.allSettled(CORE.filter(url=>!ESSENTIAL.has(url)).map(url=>fetchAndStore(cache,url)));
}
async function cached(req){const cache=await caches.open(DF_CACHE);return cache.match(cacheKey(req))}
async function networkFirst(req){const cache=await caches.open(DF_CACHE),key=cacheKey(req);try{let netReq=req;try{netReq=new Request(req,{cache:'no-cache'})}catch(e){}const res=await fetch(netReq,{cache:'no-cache'});if(res&&(res.ok||res.type==='opaque'))cache.put(key,res.clone()).catch(()=>{});return res}catch(e){const hit=await cache.match(key);if(hit)return hit;throw e}}
async function staleWhileRevalidate(req){const cache=await caches.open(DF_CACHE),key=cacheKey(req),hit=await cache.match(key);let netReq=req;try{netReq=new Request(req,{cache:'no-cache'})}catch(e){}const refresh=fetch(netReq,{cache:'no-cache'}).then(res=>{if(res&&(res.ok||res.type==='opaque'))cache.put(key,res.clone()).catch(()=>{});return res}).catch(()=>null);return hit||(await refresh)||new Response('Recurso indisponível.',{status:503})}
async function versionNetworkOnly(req){try{let netReq=req;try{netReq=new Request(req,{cache:'no-store'})}catch(e){}return await fetch(netReq,{cache:'no-store'})}catch(e){return(await cached(req))||new Response('{}',{status:503,headers:{'content-type':'application/json'}})}}
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url),same=url.origin===self.location.origin;if(!same)return;if(url.pathname.endsWith('/app-version.json')){event.respondWith(versionNetworkOnly(req));return}if(req.mode==='navigate'){event.respondWith((async()=>{try{return await networkFirst(req)}catch(e){return(await cached('./index.html'))||(await cached('./'))||new Response('DF EXTRUSOR PRO indisponível offline.',{status:503,headers:{'content-type':'text/plain; charset=utf-8'}})}})());return}event.respondWith(staleWhileRevalidate(req))});
