...
async function fetchAndStore(cache,url){const req=new Request(url,{cache:'reload'}),res=await fetch(req,{cache:'no-cache'});if(!res||(res.ok!==true&&res.type!=='opaque'))throw new Error('Falha no cache: '+url);await cache.put(cacheKey(req),res.clone());return res}
...
async function staleWhileRevalidate(req){const cache=await caches.open(DF_CACHE),key=cacheKey(req),hit=await cache.match(key);const refresh=fetch(new Request(req,{cache:'no-cache'})).then(res=>{if(res&&(res.ok||res.type==='opaque'))cache.put(key,res.clone()).catch(()=>{});return res}).catch(()=>null);return hit||(await refresh)||new Response('Recurso indisponível.',{status:503})}
async function versionNetworkOnly(req){try{return await fetch(new Request(req,{cache:'no-store'}),{cache:'no-store'})}catch(e){return(await cached(req))||new Response('{}',{status:503,headers:{'content-type':'application/json'}})}}
